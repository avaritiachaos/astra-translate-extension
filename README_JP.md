...
画像の右クリックメニュー、またはポップアップの画像選択から翻訳できます。独立した画像モデル、訳文の重ね表示、原文対照、復元、キャンセル、キャッシュ、縦長画像の分割処理に対応します。モデル ID の初期値は空欄です。サービスのモデル一覧を取得し、画像対応モデルの正確な ID を選択してください。[設定ガイド（中国語）](docs/manga-translation.md)
