...
- 🔑 **自分の API で動く** — DeepSeek とかの大模型につないで、使った分だけお支払い。1 円ぶんでも結構もつんだから。
...
- 🔒 **プライバシー最優先** — API Key はローカル保存。あたし、おにいちゃんの秘密を覗いたりしないから。……興味も、ないけど？
...
- **推論モード** — `off / low / high / xhigh / max`。DeepSeek 公式の英語値をそのまま使ってるの。
...
### プロバイダー非依存
**OpenAI-compatible** な API ならなんでも OK（DeepSeek、OpenAI、Groq、ローカルモデル……）。Base URL とモデル名は自由に書いてね。Gemini / Anthropic 対応も、そのうち。気長に待ってて♡
...
2. プロバイダープリセットで DeepSeek を選ぶ（こだわるならカスタムでも）。
3. API Key を入れる。<https://platform.deepseek.com> で登録＆チャージして作ってね。持ってないなら今日はおしまい。ざぁこ♡
4. 「接続テスト」をぽちっ。成功したら勝手に保存されるよ。
...
| 分類 | 項目 | 既定値 | メモ |
|------|------|--------|------|
| プロバイダー | プリセット / Base URL / Endpoint / モデル / API Key | DeepSeek · `deepseek-v4-flash` | OpenAI 互換 |
