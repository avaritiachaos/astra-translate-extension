...
const zhCN: MessageDict = {
  ...
  "opt.googleSearchTitle": "Google 官方搜索 API（推荐）",
  "opt.googleSearchApiKey": "Google Search API Key",
  "opt.googleSearchCx": "搜索引擎 ID (CX)",
  "opt.googleSearchHelp": "配置 Google 官方 Custom Search JSON API（每个账号每天免费 100 次）。配置后 100% 走纯正 Google 官方接口，绝不弹人机验证码，走代理节点极度稳定，且不会回退到其他引擎。",
...
const enUS: MessageDict = {
  ...
  "opt.googleSearchTitle": "Official Google Search API (Recommended)",
  "opt.googleSearchApiKey": "Google Search API Key",
  "opt.googleSearchCx": "Search Engine ID (CX)",
  "opt.googleSearchHelp": "Use official Google Custom Search JSON API (100 free queries/day per account). Immune to captchas and proxy rate-limits, purely official Google search results.",
...
const jaJP: MessageDict = {
  ...
  "opt.googleSearchTitle": "Google 公式検索 API（推奨）",
  "opt.googleSearchApiKey": "Google 検索 API Key",
  "opt.googleSearchCx": "検索エンジン ID (CX)",
  "opt.googleSearchHelp": "Google 公式 Custom Search JSON API（1 アカウントにつき毎日 100 回無料）を使用します。キャプチャ認証やプロキシ制限を受けず、常に Google 公式の検索結果を取得します。",
