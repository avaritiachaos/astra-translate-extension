...
export function mapHttpError(status: number, lang: UiLanguage = "zh-CN"): ProviderRequestError {
  switch (true) {
    case status === 401 || status === 403:
      return new ProviderRequestError(t(lang, "error.unauthorized"), "AUTH_ERROR", status);
    case status === 429:
      return new ProviderRequestError(t(lang, "error.rateLimit"), "RATE_LIMIT", status);
    case status >= 500:
      return new ProviderRequestError(t(lang, "error.serverUnavailable"), "SERVER_ERROR", status);
    default:
      return new ProviderRequestError(t(lang, "error.httpError", { status }), "HTTP_ERROR", status);
  }
}
